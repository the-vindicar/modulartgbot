...
__all__ = [
    'CoreAPI', 'PluginAPI', 'APIProvider', 'PostInit',
    'ConfigManager', 'DBModel'
]
_T = t.TypeVar('_T', infer_variance=True)
_Provider = t.Coroutine[t.Any, t.Any, _T]
# Модуль может передать этот объект после начальной инициализации,
# чтобы сообщить, что ему требуется выполнить ещё и пост-инициализацию.
PostInit = object()
...
class PluginAPI(t.Protocol):
    ...
    def lifetime(api: CoreAPI) -> t.AsyncGenerator:
        """Тело модуля. Жизненный цикл может состоять из двух или трёх этапов.
        Вариант с двумя этапами::

            async def lifetime(api: CoreAPI):
                ...  # начальная инициализация: получаем ресурсы и готовимся к работе
                yield  # управление остановится здесь на время работы системы
                ...  # завершаем работу и освобождаем ресурсы

        Вариант с тремя этапами::

            async def lifetime(api: CoreAPI):
                ...  # начальная инициализация: получаем ресурсы и готовимся к работе
                api.register_api_provider(...)  # предоставляем ресурс для последующих модулей
                yield PostInit  # даём знать, что мы хотим выполнить дополнительные действия после других модулей
                ...  # последующие модули уже выполнили свою начальную инициализацию
                ...  # мы завершаем свою с учётом того, как эти модули задействовали наш ресурс
                yield  # управление остановится здесь на время работы системы
                ...  # завершаем работу и освобождаем ресурсы

        Начальная инициализация всегда выполняется в прямом порядке (сначала независимые модули, потом зависимые).
        Пост-инициализация, если она есть, выполняется в обратном порядке (сначала зависимые, потом независимые).
        Завершение работы всегда выполняется в обратном порядке (сначала зависимые, потом независимые).

        Последний yield никогда не выбросит исключение, даже при аварийном завершении работы системы.
        """
        ...
